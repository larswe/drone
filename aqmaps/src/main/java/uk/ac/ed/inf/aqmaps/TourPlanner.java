...
public class TourPlanner {
    ...
    public int[] findShortestTour() {
        ...
        return shortestSensorTourIndices; // TODO
    }
    ...
    private int[][] computeDistanceMatrix(List<Point> points) {
        ...
        for (int i = 0; i < numPoints; i++) {
            for (int j = 0; j < numPoints; j++) {
                ...
                if (i == 0) System.out.println("i " + i + " , j " + j + " : " + drone.stepsMade);
            }
        }
    ...
    private boolean tryReverse(int i, int j) {
         ...
         */
        var oldCost = distanceMatrix[currentPointPermutation[(i - 1 + numPoints)
                % numPoints]][currentPointPermutation[i]]
                + distanceMatrix[currentPointPermutation[j]][currentPointPermutation[(j + 1) % numPoints]];
        var newCost = distanceMatrix[currentPointPermutation[(i - 1 + numPoints)
                % numPoints]][currentPointPermutation[j]]
                + distanceMatrix[currentPointPermutation[i]][currentPointPermutation[(j + 1) % numPoints]];
    ...
    private void applyTwoOptAlgorithm() {
        ...
        while (improvedTourOnPreviousLoop) {
            ...
            for (int j = 0; j < numPoints - 1; j++) {
                for (int i = 0; i < j; i++) {
                    if (tryReverse(i, j)) {
                        System.out.println(this.tourValue(currentPointPermutation));
