...
public class MainDrone extends Drone {
    ...
    public void completeTour() {
        while (1 + this.currentDestinationIndex < this.sensorTour.size()) {
    ...
    public int flyToCurrentDestination() {
        ...
        while (!this.isInRangeOfPoint(currentDestination, currentActionRange)) {
            ...
            if (this.canGetTowardsDestinationInStraightLine(currentActionRange)) {
    ...
    public boolean canGetTowardsDestinationInStraightLine(double maxFinalDistance) {
        ...
        var shadow = new ShadowDrone(this.currentPosition.longitude(), this.currentPosition.latitude(),
                this.currentDestination);

        shadow.setPosition(this.currentPosition);
        shadow.setStepsMade(0);
        
        var answer = shadow.canGetTowardsDestinationInStraightLine(maxFinalDistance);
    ...
    public void avoidObstacle(Polygon obstacle) {
        ...
        var leftShadow = new ShadowDrone(this.currentPosition.longitude(), this.currentPosition.latitude(),
                this.currentDestination);

        var approxDistanceClockwiseAvoid = leftShadow.distToAvoidObstacleClockwise(obstacle, this.currentActionRange);

        var rightShadow = new ShadowDrone(this.currentPosition.longitude(), this.currentPosition.latitude(),
                this.currentDestination);
    ...
    protected void makeMove(double angle) {
        ...
        System.out.println();
        System.out.println(this.currentDestinationIndex);
        System.out.println(this.currentPosition);
        System.out.println(this.currentDestination);
        System.out.println(this.stepsMade);
        System.out.println();
